...
import { getCampionatList, getCampionatLastPage } from "./services.ts/campionat.js";
...
import { writeFixturesBulk } from "./orm/fixture.js";
...
program
    .name('odds portal crowler')
    .description('crowler for odds portal');

program
    // .command('sca// const { Command } = require('commander');
import { Command } from "commander";
import inquirer from 'inquirer';
import { LEAGUES_URL } from "./consts/urls.js";
import { Fixture } from "./entities/fixture.js";
...
(async () => {
    ...
    initBrowser({ ...puppeteerLaunchOptions, defaultViewport: { height: 1080, width: 1920 } });
    ...
    if (previousSessions.length > 1) {
        ...
        if (continueOldSession.continueOldSession) {
            const selectedSession = await inquirer.prompt({
                ...
                choices: previousSessions.map(ps => ({ name: crawlSessiontoString(ps), value: ps }))
            });
    ...
    for (const league of leagues) {
        ...
        writeCampionati:
        for (const campionat of campionati) {
            const years = campionat.campionat.match(/\d{4}/g).map(y => +y).sort();
            ...
            writePages:
            for (let currentPageNumber = lastpaPageNr; currentPageNumber > 0; currentPageNumber--) {
